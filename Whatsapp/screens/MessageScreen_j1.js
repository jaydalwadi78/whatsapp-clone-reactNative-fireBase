...
import { registerFetchTask, contactFetchTask } from '../helpers/backgroundFetch';

import Newcontact from '../helpers/Newcontact';
...
const MessageScreen = (props) => {
  ...
  const Allcontactfilename = "Phone_contact.db";
  const Newcontactfilename = "Common_contact.db";
  const Messagesfilename = "MessagesDb.db";
 ...
 useEffect(() => {  
    ...
    // return () => subscription.remove(); 
       ...
       (async () => {
            ...
            let fileUriii =  FileSystem.documentDirectory + Messagesfilename;
          ...
          /*    let ciphertexttt = "jay";
                let fileUriii = FileSystem.documentDirectory + Messagesfilename;
                await FileSystem.writeAsStringAsync(fileUriii,ciphertexttt,{ encoding: FileSystem.EncodingType.UTF8 });
                const assettt = await MediaLibrary.createAssetAsync(fileUriii)
                await MediaLibrary.createAlbumAsync("WhatsappClone/Databases", assettt, false)*/
                ...
                if (status === "granted") { 
                     const { data } = await Contacts.getContactsAsync({ 
                      ...
                      if(data.length > 0) {
                        ...
                        console.log("Contact task>>>>>>>>>>>>>>>>>>>>MMMMMMM",data)
                        setContact(data);
                        ...
                        await MediaLibrary.createAlbumAsync("WhatsappClone/Databases", asset, false)
                        ...
                        await MediaLibrary.createAlbumAsync("WhatsappClone/Databases", assett, false)
