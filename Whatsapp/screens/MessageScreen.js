...
const MessageScreen = (props) => {
    ...
    var total_number = Object.keys(props.newcontact_data).length;
  // console.log("OOOOOOOPPPPPPPPPPPP>>>>>>",total_number);
        ...
        (async() =>{
         console.log("TTTTTTTTTTT>>>>>>>>>>>>>",props.newcontact_data)
            ...
            if (contact.length > 0 && userDetails.length > 0) {
                ...
                const myOrderedArray = _.sortBy(newcontactarray, o => o.name)
                props.newcontactCall(myOrderedArray);
            }
        })()        
    }, [isFill]);
...
useEffect(() => {
        ...
        console.log("YYYYYYYYYYYYYYY>>>>>>>>>>>>>>>>>",props)

        if (props.newcontact_data.length > 0){
            const new_cons = props.newcontact_data.map((ls, i) => ({
                key: `${i}`,
                text: `item #${i}`,
                name: `${ls.name}`,
                dbid:`${ls.dbid}`,
                expotoken: `${ls.expotoken}`,
                profile_img: `${ls.profile_img}`,
                subtitle: `${ls.number}`,
                status: `${ls.status}`
            }))
            setListData(new_cons)
        }
            ...
            <View>
                <ListItem bottomDivider>
                        <Avatar
                                    rounded
                                    size={50}
                                    source={{ uri: data.item.profile_img   }}
                                    onPress={
    ...
    return (
        <View style={styles.container}>
            ...
            <FAB
                ...
                icon="message"
                color="white"
                onPress={() => navigation.navigate('AllContact',total_number)}
            />
        </View>
    );
}
